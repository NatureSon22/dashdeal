...
const UpdateInventory = () => {
    ...
    const [previews, setPreviews] = useState([...action.image_paths.split(",")]); // images to be displayed
    const [img, setImg] = useState([...action.image_paths.split(",")][0]);
    const [startDate, setStartDate] = useState();
    const [productData, setProductData] = useState({
        productname: action.productname,
        description: action.productdescription, 
        price: Math.trunc(action.productprice),
        discountRate: Math.trunc(action.discountrate),
    ...
    useEffect(() => {
        ...
        handleDateChange(action.discountend || new Date())
        getCategories();
    }, [])
    ...
    const deleteItem = async(e) => {
        ...
        try {
            await fetch("http://localhost:3000/backend/remove-product.php", {
                method: "DELETE",
                body: JSON.stringify({ productid: action.product_id })
            });

            navigate(`/inventory`, { state: { action: 3 } });
        } catch(e) {
            console.log(e);
        }
    }
    ...
    const updateItem = async(imgPath) => {
        try {
            const res = await fetch('http://localhost:3000/backend/update-item.php', {
                method: "PUT", 
                body: JSON.stringify({ 
                    ...productData,
                    ...
                    prevRate: Math.trunc(action.discountrate)
                })
            });
    ...
    return (
        <div className="itemsell-wrapper">
            <form className="itemsell-form">
                <div className="itemsell-header">
                    <button className="inventory-btn delete" onClick={deleteItem} >Delete Item</button>
                </div>
                ...
                <div className="itemsell-container">
                    <div className="itemsell-general">
                        ...
                        <div className="button">
                            ...
                            <button onClick={() => navigate(`/inventory`, { state: { action: 3 } })}>Cancel</button>
                        </div>
                    </div>
